...
| [software-writer](./plugins/software-writer/) | Three universal, opinionated skills for writing software — writing-code (API-consultation gate, dependency shape, stack footgun check, two-tier comment policy), writing-tests (classicist behavior-first workflow), and writing-docs (single-owner surfaces, pointer-file discipline, anti-slop validation via the `human-author:ai-slop-writing-fixer` subagent) — with per-stack references for Go, Python, TypeScript, and PHP. Extendable at every step via Pre-Step-N / Post-Step-N positions and recognized named values | Development | ✅ ([custom agent](./codex-subagents/) required for writing-docs anti-slop) |
| [software-writer-extension-setup](./plugins/software-writer-extension-setup/) | One-time setup skill that explores a project's codebase, drafts project-specific overlay content conversationally, and provisions the delivery plumbing needed to extend the software-writer plugin in Claude Code or Codex, with a re-sync mode that audits overlays for drift. Long plugin and skill names prevent accidental invocation | Development | ✅ |
