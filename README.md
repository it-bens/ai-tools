...
| [work-orchestrator](./plugins/work-orchestrator/) | Opinionated orchestration of work through two delegation surfaces: dispatched workers — Codex CLI (GPT-5.6) and Claude subagents — with a task-strategy workflow, model and reasoning-effort routing, fenced write scopes, and independent two-worker verification; and sibling Claude Code sessions with session enumeration, mandatory dispatch-message blocks (SIBLING/SKILL/REPORT), topology-driven write ownership, and explicit stand-down closure. Ships 15 agent definitions pinning model and effort per duty, since Claude Code discards a spawn-time effort argument. Both skills extendable via Pre-/Post- positions and recognized named configuration values; ships the hooks that deliver a project's per-skill extension files. Renamed from subagent-orchestrator | Development | ❌ |
