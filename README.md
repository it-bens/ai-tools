...
| [llm-author](./plugins/llm-author/) | Author LLM-targeted content — prompts, skills, agents, rules files, plus session handoff and feedback prompts (model-invoked on explicit request, an invoking workflow's dispatch step, or a dispatch message's report contract; crafted via prompt-engineering) — for Claude 5 and Claude 4, GPT-5.6 (OpenAI), GLM 4.7, and Gemini 3. Delivery is shaped by recipient, so a consuming skill can obtain a prompt body alone for dispatch to a worker process, or a session-scoped ruleset to author prompts from | Productivity | ✅ |
