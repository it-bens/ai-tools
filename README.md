...
| [llm-author](./plugins/llm-author/) | Author LLM-targeted content — prompts, skills, agents, rules files, plus session handoff and feedback prompts (model-invoked on explicit request, crafted via prompt-engineering) — for Claude 5 and Claude 4, GPT-5.6 (OpenAI), GLM 4.7, and Gemini 3. Delivery is shaped by recipient, so a consuming skill can obtain a prompt body alone for dispatch to a worker process, or a session-scoped ruleset to author prompts from | Productivity | ✅ |
...
| [subagent-orchestrator](./plugins/subagent-orchestrator/) | Opinionated orchestration of implementation and review work through dispatched workers — Codex CLI (GPT-5.6) and Claude subagents — with a task-strategy workflow, model and reasoning-effort routing, fenced write scopes, and independent two-worker verification. Ships 15 agent definitions pinning model and effort per duty, since Claude Code discards a spawn-time effort argument. Extendable at five workflow positions via Pre-/Post- sections and recognized named configuration values; ships the hooks that deliver a project's extension file. The consent gate and dual-confirmation closure are deliberately fenced off from the contract | Development | ❌ |
