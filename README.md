...
| [dependency-update-triage](./plugins/dependency-update-triage/) | Read-only assessment of a single dependency update before adoption. Three prongs: required code changes, enabled improvements (including testability), and latent-bug reach from an upstream fix, each grounded in host-codebase call sites. Bounds large changesets into changed/added/fixed-bug inventories with criticality ranking and per-candidate disposition, routes work across cheap/mid/strong model tiers, and detects ecosystem/forge/updater bot at runtime | Development | ✅ (fan-out runs inline) |
...
| [llm-author](./plugins/llm-author/) | Author LLM-targeted content — prompts, skills, agents, rules files, plus session handoff and feedback prompts (model-invoked on explicit request, an invoking workflow's dispatch step, or a dispatch message's report contract; crafted via prompt-engineering) — for Claude 5 (including Opus 5.5) and Claude 4, GPT-6 and GPT-5.6 (OpenAI), GLM 4.7, and Gemini 3. Delivery is shaped by recipient, so a consuming skill can obtain a prompt body alone for dispatch to a worker process, or a session-scoped ruleset to author prompts from | Productivity | ✅ |
| [memory-triage](./plugins/memory-triage/) | Audit the project's auto-memory directory against three contracts (stale, polluting, extraction candidate), producing an approval-gated triage report with grounded verdicts, then applying approved cleanups with index and link integrity checks. A second skill lands approved extraction proposals into rule files, docs, skills, or enforcement sites with claim revalidation and routing-pointer confirmation | Productivity | ❌ |
...
| [subagent-orchestrator](./plugins/subagent-orchestrator/) | **Deprecated — renamed to [work-orchestrator](./plugins/work-orchestrator/).** Frozen copy; install the renamed plugin | Development | ❌ |
| [subagent-orchestrator-extension-setup](./plugins/subagent-orchestrator-extension-setup/) | **Deprecated — renamed to [work-orchestrator-extension-setup](./plugins/work-orchestrator-extension-setup/).** Frozen copy; install the renamed plugin | Development | ❌ |
...
| [work-orchestrator](./plugins/work-orchestrator/) | Opinionated orchestration of work through two delegation surfaces: dispatched workers — Codex CLI (GPT-5.6) and Claude subagents — with a task-strategy workflow, model and reasoning-effort routing, fenced write scopes, and independent two-worker verification; and sibling Claude Code sessions with session enumeration, mandatory dispatch-message blocks (SIBLING/SKILL/REPORT), topology-driven write ownership, and explicit stand-down closure. Ships 15 agent definitions pinning model and effort per duty, since Claude Code discards a spawn-time effort argument; ten of them accept opt-in nested subagent delegation, stated explicitly in every dispatch. Both skills extendable via Pre-/Post- positions and recognized named configuration values; ships the hooks that deliver a project's per-skill extension files. Renamed from subagent-orchestrator | Development | ❌ |
| [work-orchestrator-extension-setup](./plugins/work-orchestrator-extension-setup/) | One-time setup skill that explores a project's gates, protected paths, conduct rules, recurring checkpoint types, and session topology, drafts the extension content conversationally, and writes the per-skill extension files for the work-orchestrator plugin, with a re-sync mode that audits them for drift. Long plugin and skill names prevent accidental invocation. Renamed from subagent-orchestrator-extension-setup | Development | ❌ |
