#!/usr/bin/env bats
# bats file_tags=subagent-orchestrator
...
@test "post-tool-use: ignores a same-plugin prefix that is not the skill" {
    make_extension "## Post-Dispatch"
    run_post_tool_use "subagent-orchestrator:orchestrating"
    assert_success
    assert_output ""
}
...
@test "post-tool-use: ignores the companion setup plugin's skill" {
    make_extension "## Post-Dispatch"
    run_post_tool_use "subagent-orchestrator-extension-setup:setting-up-subagent-orchestrator-extension"
    assert_success
    assert_output ""
}
...
@test "user-prompt: ignores the companion setup plugin's slash command" {
    make_extension "## Post-Dispatch"
    run_user_prompt "/subagent-orchestrator-extension-setup:setting-up-subagent-orchestrator-extension"
    assert_success
    assert_output ""
}
...
@test "user-prompt: ignores the slash command without an extension file" {
    run_user_prompt "/subagent-orchestrator:orchestrating-subagent-work review the branch"
    assert_success
    assert_output ""
}
...
@test "user-prompt: ignores a slash command with trailing characters in the skill name" {
    make_extension "## Post-Dispatch"
    run_user_prompt "/subagent-orchestrator:orchestrating-subagent-workx"
    assert_success
    assert_output ""
}
...
@test "user-prompt: ignores the slash command when it is not at the start of the prompt" {
    make_extension "## Post-Dispatch"
    run_user_prompt "run /subagent-orchestrator:orchestrating-subagent-work please"
    assert_success
    assert_output ""
}
...
- \`project.gates\` = go test ./..."
    ...
    [[ "$context" == '<project_extension skill="subagent-orchestrator:orchestrating-subagent-work" position="after-skill-body">'* ]]
...
List the project gates before stating the strategy."
    ...
    [[ "$context" == '<project_extension skill="subagent-orchestrator:orchestrating-subagent-work" position="before-skill-body">'* ]]
...
@test "user-prompt: delivers the extension for a bare slash command" {
    ...
    run_user_prompt "/subagent-orchestrator:orchestrating-subagent-work"
    assert_success

    local context
    context=$(printf '%s' "$output" | jq -r '.hookSpecificOutput.additionalContext')
    [[ "$context" == *'skill="subagent-orchestrator:orchestrating-subagent-work"'* ]]
...
@test "user-prompt: ignores a cwd extension file when CLAUDE_PROJECT_DIR is unset" {
    make_extension "## Pre-Strategy"
    local json
    json=$(jq -n --arg cwd "$CLAUDE_PROJECT_DIR" '{prompt: "/subagent-orchestrator:orchestrating-subagent-work review", cwd: $cwd}')
...
@test "post-tool-use: self-gates silently when CLAUDE_PROJECT_DIR is unset" {
    run bash -c 'unset CLAUDE_PROJECT_DIR; printf "%s" "{\"tool_input\": {\"skill\": \"subagent-orchestrator:orchestrating-subagent-work\"}, \"cwd\": \"/tmp/project\"}" | bash "$1" post-tool-use' _ "$SCRIPT"
    assert_success
    assert_output ""
}
