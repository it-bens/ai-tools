#!/usr/bin/env bats
# bats file_tags=file-read,plugin-launch
...
@test "Codex MCP launcher uses plugin-root cwd and a direct entrypoint" {
    run jq -e '
        .mcpServers["file-read"] |
        .command == "bash" and
        .args == ["./mcp-server-read/server.sh"] and
        .cwd == "." and
        .env == {}
    ' "${MANIFEST}"
    assert_success
    ...
    [[ "${manifest}" != *"./plugins/file-read"* ]]
}

@test "Codex MCP launcher initializes from its declared cwd" {
    run bash -c '
        cd "$1"
        printf "%s\n" "$2" | bash ./mcp-server-read/server.sh
    ' _ "${PLUGIN_DIR}" "${INITIALIZE_REQUEST}"
    assert_success
    jq -e '
        .id == 1 and
        .result.serverInfo.name == "file-read" and
        .result.protocolVersion == "2024-11-05"
    ' <<<"${output}" >/dev/null
}

@test "SessionStart hook uses only host-provided plugin roots" {
    local command
    command=$(jq -r '.hooks.SessionStart[0].hooks[0].command' "${HOOKS_CONFIG}")

    [[ "${command}" == *"PLUGIN_ROOT"* ]]
    [[ "${command}" == *"CLAUDE_PLUGIN_ROOT"* ]]
    [[ "${command}" != *"CODEX_PLUGIN_ROOT"* ]]
    [[ "${command}" != *"./plugins/file-read"* ]]
    [[ "${command}" != *"./hooks/scripts"* ]]
}
...
@test "SessionStart hook fails when the host omits the plugin root" {
    local hook_command
    hook_command=$(jq -r '.hooks.SessionStart[0].hooks[0].command' "${HOOKS_CONFIG}")
    ...
    ' _ "${hook_command}"
    assert_failure
    assert_output --partial "file-read: plugin root is unavailable"
}
