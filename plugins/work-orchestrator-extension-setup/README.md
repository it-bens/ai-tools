...
**What it explores.** Gate commands as actually declared in Makefiles, package scripts, and CI workflows, with their sandbox behavior; generated trees, lockfiles, and migration history for the write fence; command classes that touch shared infrastructure or cost money; conduct rules and project skill files a worker must receive; recurring defect classes that become review lenses; work types the universal routing table has no profile for; and — for the session skill — the working-tree and branch facts behind write ownership, while roles and duties come from you conversationally.
