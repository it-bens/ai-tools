...
- Built-in allow-path rules (`PULLMD_BUILTIN_ALLOW_PATHS` in `lib.sh`) for JSON APIs served from hosts that also serve real pages. Each rule pairs a host (exact or subdomain, like the built-in allow list) with a path regex, so only matching URLs skip PullMD while the host's pages still route through it. Ships with PyPI's package JSON API (`pypi.org/pypi/<pkg>/json` and `/pypi/<pkg>/<version>/json`, covering `test.pypi.org`); a `pypi.org` project or search page is still redirected to PullMD. Previously these JSON endpoints had to burn the escape hatch to get through. Backed by new `lib.sh` helpers `extract_path` and `url_allowed`.
