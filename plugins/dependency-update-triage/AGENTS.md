...
**No commands, agents, hooks, scripts, or MCP servers.** All companions (git-forge tooling, web reader, clipboard) are soft: the skill checks for each and degrades with a note when it is absent rather than depending on it.
...
| Modify the report shape or companion degradation | `skills/triaging-a-dependency-update/references/output.md` | Report sections, clipboard delivery, degrade-with-a-note table |
...
No scripts or hooks, so no BATS coverage. Validate skill-body changes by invoking the skill against a sample update in a scratch project: confirm it routes gather → detect → investigate → report, produces the three-pronged output shape, grounds every impact claim in a code reference, and delivers to the clipboard without editing the repo.
