...
| Skill                   | What it does                                                                                                                                                                                                                                                                                                                         |
|-------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `changelog-summarizing` | Generate Discord and Slack summary posts of repository changes since a given date. Walks committed history on `main`, groups commits by conventional-commit scope, synthesizes each group into a contextual paragraph, and dispatches the `human-author:ai-slop-writing-fixer` subagent before producing platform-formatted outputs. |
...
- **Dispatches the `human-author:ai-slop-writing-fixer` subagent.** Em dashes, banned vocabulary, formulaic transitions, and metronomic sentence rhythm get caught and rewritten before output. The agent ships with the `human-author` plugin, which is declared as a dependency and auto-installs when you install this one.
