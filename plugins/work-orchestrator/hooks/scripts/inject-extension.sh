...
PLUGIN="work-orchestrator"
SKILLS=("orchestrating-subagent-work" "orchestrating-session-work")
...
if [[ "$MODE" == "post-tool-use" ]]; then
    ...
    for CANDIDATE in "${SKILLS[@]}"; do
        if [[ "$FIELD" == "${PLUGIN}:${CANDIDATE}" ]]; then
    ...
    for CANDIDATE in "${SKILLS[@]}"; do
        if [[ "$FIELD" =~ ^/${PLUGIN}:${CANDIDATE}([[:space:]]|$) ]]; then
            SKILL_NAME="$CANDIDATE"
            break
        fi
    done
...
EXTENSION_FILE="${CLAUDE_PROJECT_DIR}/.claude/extensions/${PLUGIN}/${SKILL_NAME}.md"
