...
A replication run on a fresh fixture — seven planted defects plus a comment-drift canary, hidden answer key and acceptance tests, Codex CLI 0.156.1, one run per codex cell (six each for the gpt-5.6-sol, gpt-6-sol, and gpt-5.6-terra broad reviews) and two per Claude rung — did not reproduce five per-model GPT-5.6 observations the routing table relied on: Sol labeling every finding blocking, Luna as the only source of new findings, Luna's labels as the most conservative and correct, Terra reporting no test-quality findings, and Terra as the only implementer to re-derive the true scope. Four security reviews, Sol included, met no safeguard friction on a scope without offensive-security content. Capability statements across the plugin are recalibrated to state their sample, and routing rules that rested on single runs become model-neutral working defaults.

### Changed

- `skills/orchestrating-subagent-work/references/model-routing.md` — the intro states the rows are working defaults drawn from small samples; the optional third broad pass with `gpt-5.6-luna` adds a third model's coverage instead of claiming a doc-drift and installed-dependency specialty; the security-review row drops "(never sol)"; the safeguard-refusal rule covers every worker, with codex refusals re-routed to a different codex model, a claude definition, or the session; the haiku rule states the design reason (no decisions, so its output reaches one only through verification) instead of a trait; the per-model severity-label rule becomes one model-neutral rule — every codex model's labels are provisional and re-ranked before triage; the stronger-model-at-a-lower-rung preference is phrased as the default
- `skills/orchestrating-subagent-work/references/codex-dispatch.md` — the `exec resume` flag sentence is policy rather than a closed-set claim: a resume passes only `--model`, `-c`, and `--disable`, and `--sandbox` and `-C` are rejected
- `docs/codex-dispatch-experiments.md` — a §Replication, 2026-09-24 section records the setup, results, the five non-replicated observations, and the confounds; §Model roles bullets gain their sample size where the series recorded one, and the replication result where the replication covered them; the sol/luna test-quality observation drops "only" and states that its sol/luna run count was not recorded, beside terra's six; the replication table gains a row with the per-run Claude implementer token counts (sonnet-high 142,926 / 151,973; opus-medium 105,224 / 107,062) backing `claude-effort-mechanism.md`'s ~147k / ~106k figures; §Transport records that CLI 0.156.1's `exec resume` accepts more flags than 0.144.4's while still rejecting `--sandbox` and `-C`; the GPT-6 transfer note no longer names the dropped Sol exclusion
- `docs/claude-effort-mechanism.md` — the no-opus-`low` reasoning states its MCPMark basis is GPT-5 against GPT-5-mini and that "sonnet at `medium` covers that cell" is an untested maintainer default, with the local `implement-opus-medium` versus `implement-sonnet-high` data point (n=2, one task); the Opus 5.5 material gains the medium/high/xhigh rung proxy (identical outcomes, n=2 per rung, one task, token and time per rung), with no routing change
- `docs/subagent-report-shape.md` — the opus verdict-first result is stated as one unedited run, and the haiku phrasing result as four positive-phrasing runs against one negative-phrasing run
- `docs/gpt-5-6-model-family.md` — Sol > Terra > Luna is labeled as OpenAI's positioning; the Sol safeguard basis records that the replication saw no friction and that the Sol exclusion was dropped while the safeguard risk remains untested
- `README.md` — the no-opus-`low` sentence names it a maintainer default untested on Claude; the `docs/codex-dispatch-experiments.md` entry mentions the replication; the `docs/subagent-report-shape.md` entry states the haiku result as its one-run-versus-four-run sample instead of a trait
- `CLAUDE.md` — a maintenance rule: per-model capability statements in `docs/` carry their sample size and conditions, `references/` and `SKILL.md` phrase per-model behavior as working defaults, and a rule resting on one run is re-checked before it is tightened
- `skills/orchestrating-subagent-work/references/model-routing.md` — the `routing.additions` append-only rule gives its reason as the rows being the plugin's defaults, each with a recorded basis in the plugin's docs, instead of claiming the rows carry the plugin's evidence base; the append-only rule itself is unchanged
- `docs/codex-dispatch-experiments.md` — a §Routing-table basis (codex rows) section gives each codex row of the routing table, and each closest-profile fallback, its basis kind (vendor positioning, cost, local observation with its n, or maintainer judgment), the evidence behind it, and what would change it; rows resting on the original single runs state what the replication showed; §Replication and the replication clauses in §Model roles state counts instead of ranking words (the D7 label as 1 of 6 broad reviewers correct, valid-new findings per model with the zeros named, canary 6 of 6, fence and gate claims 12 of 12)
- `docs/claude-effort-mechanism.md` — a §Routing-table basis (claude rows) section gives each claude row of the routing table its rung and model, basis kind (Opus 5 vendor measurement, vendor guidance, local observation with its n, model-family diversity, cost, or maintainer judgment), the evidence behind it, and what would change it
- `CLAUDE.md` — the "Consult the evidence behind a directive" navigation row points at both routing-table basis sections
- `docs/codex-dispatch-experiments.md` — a §Stability subsection records six identical broad reviews each for gpt-5.6-sol, gpt-6-sol, and gpt-5.6-terra: key-item recall and labels are stable within each model (D7 over-rated in 17 of 18 runs, D6 over-rated by gpt-5.6-sol only), findings beyond the key vary 0 to 4 per run within one model and total 7, 5, and 0; §Model roles, §Replication, and the broad-review and optional-third-pass basis rows state what single runs and six runs can and cannot separate
- `docs/nested-subagent-delegation.md` — the haiku roster row cites the routing table's design reason (no decisions, so its output reaches one only through verification) instead of a trait, and states the trait claim was never measured

No named value, position, or extension-file format changed. Projects with an extension file need no action.

## [4.5.0] - 2026-09-23

Opus 5.5 shipped with a default reasoning effort of `medium`, not the `high` every other effort-capable Claude model defaults to — and Claude Code's `opus` alias resolves to `claude-opus-5-5` on 2.1.280, confirmed by `message.model` in this session's own subagent transcripts. The opus rung measurements this plugin's agent set is built on come from the Opus 5 system card and are unvalidated on Opus 5.5; Anthropic's own guidance states effort names are not calibrated the same way across the two models, with Opus 5.5 at `medium` matching or beating Opus 5 at `high` on coding evaluations. OpenAI released GPT-6 Sol and GPT-6 Luna on 2026-09-22; GPT-6 is `gpt-6-astra` / `gpt-6-sol` / `gpt-6-luna`, with no Terra, and GPT-5.6's own price table in `docs/gpt-5-6-model-family.md` had drifted from the pricing page.

### Changed

- `docs/claude-effort-mechanism.md` — the default-effort statement is now per-model (`high` on Opus 5, Sonnet 5, Fable 5.1; `medium` on Opus 5.5), records the `opus` alias's resolution to `claude-opus-5-5` on Claude Code 2.1.280, and marks the Opus 5 system-card rung measurements as unvalidated on Opus 5.5, quoting Anthropic's medium-matches-high finding; the `max`-ships-on-nothing reasoning gains Opus 5.5's higher per-turn thinking at a given rung and two independent token-volume measurements; the sources list gains the Opus 5.5 doc URLs; a dated 2026-09-23 paragraph records the vendor's Opus 5.5 safeguard-refusal statements on biology and cybersecurity and the migration guide's broader-classifier-set statement, plus three anecdotal Reddit refusal reports for the same period
- `agents/implement-opus-high.md`, `agents/implement-opus-medium.md`, `agents/investigate-opus-medium.md` — the description no longer claims a "model default" or "measured highest" rung; they now read "high depth" and "medium depth", matching the sonnet definitions' wording
- `skills/orchestrating-subagent-work/SKILL.md` — the worker-family sentence states that opus definitions resolve to Opus 5.5, a later Claude 5 release whose effort names are not calibrated like Opus 5's
- `skills/orchestrating-subagent-work/references/model-routing.md` — the routing rules gain a bullet: a safeguard refusal from a claude worker is off-profile output that re-routes the checkpoint to the table's codex actor for that scope type, to a different claude model, or back to the session itself, and security code review is not excluded from opus since the classifiers exclude source-code vulnerability finding from the cybersecurity refusal category, though they can decline life-sciences biology work
- `README.md` — the Opus-rung-sensitivity sentence notes the measurement is Opus 5's and is unvalidated on Opus 5.5
- `docs/gpt-5-6-model-family.md` — a dated note records the 2026-09-22 GPT-6 Sol/Luna release, the GPT-6 tier lineup and pricing, and the Codex-side "remain available during the rollout" statement and starting efforts; the GPT-5.6 price table is corrected to the pricing page's current values ($4/$20 sol, $2/$12 terra, $0.20/$1.20 luna) and notes Sol's promotional-pricing end date; the sources list gains the GPT-6 model-page and Codex-docs URLs, plus the Codex changelog (https://learn.chatgpt.com/docs/changelog) and GitHub changelog (https://github.blog/changelog/2026-09-22-openais-gpt-6-sol-and-gpt-6-luna-now-available/) entries sourcing the 2026-09-22 release date. The routing table's codex rows are unchanged by intent: GPT-6 tiers do not map onto the GPT-5.6 rows by name, and re-validating the routing table against GPT-6 is pending work, not a decision to skip
- `docs/codex-dispatch-experiments.md` — a line in Named confounds states every result is GPT-5.6 at the stated effort on Codex CLI 0.144.4 and does not transfer to GPT-6 by tier name
