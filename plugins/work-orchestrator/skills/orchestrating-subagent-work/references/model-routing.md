...
Assign every checkpoint an actor from this table. Codex actors are model tiers; claude actors are the agent definitions in `agents/`. When a task type is absent, route by the closest profile: open-ended judgment → gpt-5.6-sol; specified everyday work → gpt-5.6-terra or an `implement-sonnet-*` definition; enumerated repeatable work → gpt-5.6-luna or a haiku definition. When a model tier is updated, re-validate this table before relying on it; its rows are working defaults drawn from small samples, not measured rankings.

`routing.additions` appends checkpoint-type rows for project checkpoint types this table has no profile for; default if not otherwise stated: none. Additions append only — a row here never rewrites one below, because the rows below are the plugin's defaults, each with its recorded basis in the plugin's docs, so an addition cannot silently override one.
...
| Optional third broad pass, adding a third model's coverage to the broad pair | codex `gpt-5.6-luna` | high |
| Review of security- or privacy-flavored scopes | codex `gpt-5.6-terra` | high |
...
- Haiku output never flows into a decision unverified: pair every haiku fan-out with a sonnet verification stage or a deterministic check (the gate re-run's exit-code acceptance is such a check). Give haiku decision-free instructions and a structured output schema: haiku gets no decisions, so its output reaches a decision only through verification.
- Every codex model's severity labels are provisional: re-rank findings by the orchestrator's own assessment before triage.
...
- Escalation criterion when a result is weak: a worker that skipped part of the scope or stopped partway needs a higher rung; a worker that had everything it needed, tried, and still got it wrong needs a stronger model. By default, prefer a stronger model at a lower rung over a weaker model at a higher one.
- A safeguard refusal from any worker, codex included — a declined request in place of the duty's output — is off-profile output, not a result: re-route a claude refusal to the codex actor this table names for that scope type, to a different claude model, or back to the session itself; re-route a codex refusal to a different codex model, to a claude definition, or back to the session itself; never accept the decline as the verdict. The opus definitions' classifiers can decline life-sciences biology work (everyday health and educational questions are unaffected) and high-risk dual-use cybersecurity activity but leave source-code vulnerability finding allowed, so security code review is not excluded from opus.
