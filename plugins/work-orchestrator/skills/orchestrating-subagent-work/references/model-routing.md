...
- A safeguard refusal from any claude worker — a declined request in place of the duty's output — is off-profile output, not a result: re-route the checkpoint to the codex actor this table names for that scope type, to a different claude model, or back to the session itself, never accept the decline as the verdict. The opus definitions' classifiers can decline life-sciences biology work (everyday health and educational questions are unaffected) and high-risk dual-use cybersecurity activity but leave source-code vulnerability finding allowed, so security code review is not excluded from opus.
