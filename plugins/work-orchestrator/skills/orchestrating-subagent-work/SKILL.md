...
The session is the orchestrator: it owns specs, triage, adjudication, git, and write authority — and it conserves its own context by not re-doing worker verification. Workers never write by default; a worker writes only inside a write scope its checkpoint explicitly fences — plus the report file its dispatch names — and every worker-written change is diff-reviewed by an independent worker before it stands. Dispatched workers (codex, sonnet/haiku subagents) are stateless specialists that receive self-contained prompts; their results are confirmed by a second, independent worker, not by the orchestrator. A result confirmed by two workers (codex counts as a worker) is final — the orchestrator verifies something itself only when a deviation routes it there. No worker is dispatched before the workflow below reaches "Execute next strategy step". Sibling sessions are not dispatched workers: distributing work across sessions is `work-orchestrator:orchestrating-session-work`'s domain, and this skill governs the work executed inside a session.
...
2. Per checkpoint: actor (codex model / agent definition / session), effort, and dispatch order (codex dispatches sequential; read-only subagent fan-outs may run parallel to a background codex run) — plus any routing-table optional pass being omitted, and, per checkpoint whose actor's definition permits the Agent tool, whether its dispatch delegates nested subagents. Nested delegation is off by default; declare it on only when the user asked for it in this conversation or the checkpoint's scope makes the delegable legwork plain, and name that reason. Either way the dispatch states the choice explicitly.
...
Every implementer dispatch names the worker's report file per that reference's §Report files. Adjudicate on the worker's final verdict — per-item status, deviations, the not-verified list. Read a report file only when a deviation trigger fires or the result is disputed; a confirmer dispatch carries the producer's report file path and reads it itself.
...
- Subagent spawns: dispatch the agent definition the routing table names; it carries the model and the reasoning effort. Never set an effort on the spawn itself — to change the rung, dispatch a different definition. Spawn the worker without a name — a named worker's final message does not arrive on its own. A checkpoint that genuinely needs a named worker states sending the final message as that worker's last action, inside its prompt's output contract (REPORT for implementers, OUT for reviewers). When a worker finishes and its final message has not arrived, extract only the final assistant block from that worker's persisted transcript under `~/.claude/projects/<project-slug>/<session-id>/subagents/`; never read such a transcript whole. A worker's claim to have sent its final message never substitutes for the message itself. Every subagent dispatch carries the nested-subagent directive per that reference's §Nested subagents — the strategy's per-checkpoint declaration picks the form, and absent a declaration the form is spawn-none. Include the honesty and fail-hard directives the worker must follow; workers inherit nothing from the session. The orchestrator stays the sole file writer unless a checkpoint explicitly fences a worker's write scope; a dispatch's named report file is always an allowed worker write.
