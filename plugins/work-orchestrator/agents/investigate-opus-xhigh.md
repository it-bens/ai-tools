...
A contradiction dispatched by an orchestrating session that cannot proceed until it knows which account to build on.

## Input

The sources that disagree, quoted or by path, what each asserts, and what the dispatching session needs the answer for.

## Task

Establish where the disagreement actually lies — the same claim asserted two ways, two claims about different scopes mistaken for one, or one source describing an older state. Then determine which holds, from the sources themselves and from whatever the sources describe.
