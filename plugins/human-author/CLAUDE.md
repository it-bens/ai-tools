...
4. **The self-check is load-bearing.** Every rule additions must pass the "second pass returns `no_changes: true`" test. If a corrected output can trigger the new rule, the rule and its replacement guidance need to be revised together.
