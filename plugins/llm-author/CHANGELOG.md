...
Anthropic released Claude Opus 5.5 and made it the recommended starting model; OpenAI released the GPT-6 family (Astra, Sol, Luna). The `prompt-engineering` guides described Opus 5, Sonnet 5, and Fable 5 as the current Claude lineup with `high` as the universal effort default, and GPT-5.6 as the current OpenAI generation. Opus 5.5 defaults to `medium`, rejects forced `tool_choice`, and adds the `reasoning_extraction` refusal; GPT-6 drops the Terra tier, puts Astra above Sol, and ships prompting guidance observed on Astra that OpenAI applies to the whole family as a starting point to evaluate per model.

Sources: [Prompting Claude Opus 5.5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5-5), [What's new in Claude Opus 5.5](https://platform.claude.com/docs/en/models/opus-5-5/whats-new-opus-5-5), [Migrating to Claude Opus 5.5](https://platform.claude.com/docs/en/models/opus-5-5/migration-guide), [Models overview](https://platform.claude.com/docs/en/models/overview), [Using GPT-6](https://developers.openai.com/api/docs/guides/latest-model), the [GPT-6 Sol](https://developers.openai.com/api/docs/models/gpt-6-sol), [Luna](https://developers.openai.com/api/docs/models/gpt-6-luna), and [Astra](https://developers.openai.com/api/docs/models/gpt-6-astra) model pages, [Rethinking skills and prompts for GPT-6 Astra](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra), and the Codex [subagents](https://learn.chatgpt.com/docs/agent-configuration/subagents) and [models](https://learn.chatgpt.com/docs/models) pages. Independent coverage read alongside them: [Artificial Analysis on Claude Opus 5.5](https://artificialanalysis.ai/articles/claude-opus-5-5), [Simon Willison on Opus 5.5, Sol, and Luna](https://simonwillison.net/2026/Sep/22/opus-and-sol-and-luna/), [Artificial Analysis on GPT-6 Sol and Luna](https://artificialanalysis.ai/articles/gpt-6-sol-and-luna-push-the-cost-efficiency-frontier). Every statement in the skill rests on the vendor pages.

### Added

- `references/claude-5-guide.md` — "Opus 5.5 — recommended default" per-model section: Opus 5 prompts carry over, `medium` default with thinking always on, more thinking per turn (`max_tokens` up to 128,000 at `xhigh`/`max`), text-only `end_turn` treated as a progress report in unattended loops, the `pasted_content` pattern for pasted text (tags can be imitated), and no instructions to write out internal reasoning; closes with an Opus 5 → Opus 5.5 checklist
- `references/openai-gpt-guide.md` — GPT-6 model table (Astra, Sol, Luna; no Terra), Codex subagent starting efforts, a "GPT-6 Behavior and How to Prompt It" section labeled as observed on Astra, and a "GPT-5.6 → GPT-6" migration path
- `examples/openai-gpt-adaptation.md` — Example 4: a blanket GPT-5.6-era `AGENTS.md` block rewritten as conditional doc pointers, bounded re-verification, and standing permission for a safe local test workflow
- `docs/` — nine vendor pages saved verbatim: the three Opus 5.5 pages, the models overview, Using GPT-6, the three GPT-6 model pages, and the GPT-6 Astra skills article

### Changed

- `references/gpt-56-guide.md` → `references/openai-gpt-guide.md` and `examples/gpt-56-adaptation.md` → `examples/openai-gpt-adaptation.md`; GPT-5.6 content is kept and labeled, including `text.verbosity`, pro mode, PTC, and multi-agent guidance as verified for GPT-5.6. The autonomy-boundary section and its confirmation template now apply to GPT-6 agentic prompts too. "Migrating from Earlier GPT-5.x" became "Migrating to GPT-6", with the GPT-5.x → GPT-5.6 steps and hazards kept beneath it
- `references/claude-5-guide.md` — lineup names Opus 5.5 and Fable 5.1 as current; the effort default is stated per model, and the re-sweep line says level names do not carry across models; the breaking-changes table extends the `thinking: {type: "disabled"}` row to Fable 5.1 and Opus 5.5 and adds forced `tool_choice`; Model Selection adds Opus 5.5 as the starting point and Fable 5.1 as the escalation tier; the Fable reasoning-echo bullet names the `reasoning_extraction` refusal on Fable 5 and Opus 5.5
- `SKILL.md` (prompt-engineering) — description triggers cover GPT-6 and Opus 5 → Opus 5.5 migration; the digraph's OpenAI node points at the renamed guide; the target-model list, Claude 5 lineup and breaking-changes line, and the OpenAI adaptation section cover Opus 5.5 and GPT-6
- `references/output-formats.md` — sections 11 and 12 cover GPT-6 and GPT-5.6 (anchors renamed); model lists there and in the system-prompt and prompt-chain templates add Opus 5.5, Fable 5.1, and GPT-6 or GPT-5.6
- `references/techniques/chain-of-thought.md`, `references/techniques/prefilling.md`, and `examples/claude-4-to-5-migration.md` — Opus 5.5 refusal risk and prefill 400, per-model effort default, and an Opus 5.5 checklist line
- All five skills' frontmatter `version` is 3.13.0, fixing the earlier skew (four at 3.11.0, one at 3.12.0)
- Manifests, `AGENTS.md`, `README.md`, and `project/` name GPT-6 and Opus 5.5 and point at the renamed files
