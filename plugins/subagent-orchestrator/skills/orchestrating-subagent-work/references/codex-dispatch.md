...
| SKILLS | Project skill files as required reading, when the scope warrants them. |
| LENS | Three to four named review lenses for the scope. |
...
| FENCE | Named allowed files; no commit/stage/push; banned git verbs on a dirty tree; protected paths; banned command classes (e2e suites, containers, device tooling, deployments). |
| SKILLS | Project skill files as required reading. |
| RULES | Verbatim extracts of the project's conduct rules: fail-hard (no silent fallbacks), calibrated honesty (never claim an unrun gate passed), doc drift (every touched doc claim verified in the same change). |
...
| GATES | The project's verification gates with exact commands (format, typecheck, lint, test). Name any gate known to fail inside the sandbox and its one permitted fallback. Name the gates an independent worker re-runs outside the sandbox after the run returns. |
